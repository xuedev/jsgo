...
const top = {};
top.time = new Date().getTime();

const callp = function(plugin,version,func,param){
  var p = {};
  p.p = plugin;
  p.v = version;
  p.f = func;
  p.param = JSON.stringify(param);
  return eval("("+go.run(JSON.stringify(p))+")");
}
const calla = function(p){
  var param = p.param;
  p.param = JSON.stringify(param);
  return eval("("+go.callApi(JSON.stringify(p))+")");
}

const callar = function(p){
  var param = p.param;
  p.param = JSON.stringify(param);
  return go.callApi(JSON.stringify(p));
}

top.dbInit = function(db,url){
  var p = {};
  p.p = "db";
  p.v = "1.0";
  p.f = "DbInit";
  ...
  dbp.db = db;
  dbp.url = url;
  p.param = JSON.stringify(dbp);
  return eval("("+go.run(JSON.stringify(p))+")");
}
const dbInit = top.dbInit;

top.dbQuery = function(db,sql){
  var p = {};
  p.p = "db";
  p.v = "1.0";
  p.f = "DbQuery";
  ...
  q.db = db;
  q.sql = sql;
  p.param = JSON.stringify(q);

  return eval("("+go.run(JSON.stringify(p))+")");
}
const dbQuery = top.dbQuery;
